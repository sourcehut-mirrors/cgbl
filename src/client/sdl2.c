...
static const SDL_GameControllerButton BUTTON[CGBL_BUTTON_MAX] = { SDL_CONTROLLER_BUTTON_A,          SDL_CONTROLLER_BUTTON_B,
                                                                  SDL_CONTROLLER_BUTTON_BACK,       SDL_CONTROLLER_BUTTON_START,
                                                                  SDL_CONTROLLER_BUTTON_DPAD_RIGHT, SDL_CONTROLLER_BUTTON_DPAD_LEFT,
                                                                  SDL_CONTROLLER_BUTTON_DPAD_UP,    SDL_CONTROLLER_BUTTON_DPAD_DOWN };

static const SDL_Scancode KEY[CGBL_BUTTON_MAX] = { SDL_SCANCODE_X,     SDL_SCANCODE_Z,    SDL_SCANCODE_C,  SDL_SCANCODE_SPACE,
...
static cgbl_error_e cgbl_client_audio_create(void) {
    SDL_AudioSpec specification = { .freq = CGBL_AUDIO_SAMPLES * CGBL_CLIENT_FRAME_RATE, .format = AUDIO_F32, .channels = 1 };
...
static cgbl_error_e cgbl_client_video_create(uint8_t scale, bool fullscreen) {
    ...
    if (!(client.video.renderer = SDL_CreateRenderer(client.video.window, -1,
                                                     SDL_RENDERER_ACCELERATED | (CGBL_CLIENT_VSYNC ? SDL_RENDERER_PRESENTVSYNC : 0)))) {
